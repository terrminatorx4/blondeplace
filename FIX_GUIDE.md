...
1. **Git конфликты при параллельной работе** - до 20 потоков пытались одновременно делать Git операции
2. **Некорректная проверка изменений** - `git status --porcelain` работала неправильно
3. **Отсутствие retry логики** для Git push операций
4. **Недостаточная обработка ошибок API**

## ✅ Внесенные исправления

### 1. Исправлен Git workflow (`.github/workflows/factory.yml`)

**Было:**
```yaml
git add src/content/posts/*.md
git commit -m "..."
git pull --rebase  # ❌ Конфликты!
git push
```

**Стало:**
```yaml
# Правильная проверка новых файлов
NEW_FILES=$(git ls-files --others --exclude-standard src/content/posts/*.md 2>/dev/null || true)
MODIFIED_FILES=$(git diff --name-only src/content/posts/*.md 2>/dev/null || true)
