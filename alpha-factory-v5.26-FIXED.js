// ===== ALPHA-FACTORY v5.26-FIXED - РАБОЧАЯ ВЕРСИЯ + baseNumber = 200000 =====
// ОСНОВА: Рабочая v5.26 (все работает) + ТОЛЬКО baseNumber = 200000 для sitemap fix!
// НИКАКИХ других изменений - только обход GitHub API лимита!
...
// 7. ПЕРЕСПАМ: Экстремально агрессивные инструкции для AI ✅
// 8. АНКОРЫ: Ключевые слова вместо "читать", "узнать" ✅
// 9. HTML: Исправлено попадание тегов в семантическое ядро ✅
// 10. СЛАБЫЕ СТАТЬИ: Усилены до доминирования ✅
// 11. AI ИНСТРУКЦИИ: Кардинально усилены для переспама ✅
...
async function getNextAvailablePostNumber(threadId) {
    try {
        console.log(`[NUMBERS] Thread #${threadId}: Получаю следующий доступный номер поста...`);
        ...
        console.log(`[NUMBERS] Thread #${threadId}: Базовый номер: ${baseNumber}`);
        console.log(`[NUMBERS] Thread #${threadId}: Кандидат на номер: ${candidateNumber}`);
        
        // ⚡ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА СУЩЕСТВОВАНИЯ ФАЙЛА
        ...
        try {
            ...
            console.log(`[CHECK] Thread #${threadId}: Запрос к GitHub API: ${fileUrl}`);
            ...
            console.log(`[CHECK] Thread #${threadId}: Ответ GitHub API: ${response.status}`);
            
            if (response.status === 200) {
                console.log(`[CHECK] Thread #${threadId}: ⚠️⚠️⚠️ ФАЙЛ post${candidateNumber}.md УЖЕ СУЩЕСТВУЕТ!`);
                console.log(`[CHECK] Thread #${threadId}: 🔄 ОБЯЗАТЕЛЬНО ищу следующий доступный номер...`);
                
                // Ищем следующий доступный номер
                let nextNumber = candidateNumber;
                let attempts = 0;
                const maxAttempts = 100; // Увеличиваем попытки
                
                while (attempts < maxAttempts) {
                    nextNumber += 1;
                    attempts++;
                    
                    console.log(`[CHECK] Thread #${threadId}: Попытка ${attempts}: проверяю post${nextNumber}.md...`);
                    ...
                    console.log(`[CHECK] Thread #${threadId}: post${nextNumber}.md статус: ${nextResponse.status}`);
                    
                    if (nextResponse.status === 404) {
                        console.log(`[CHECK] Thread #${threadId}: ✅✅✅ НАЙДЕН СВОБОДНЫЙ НОМЕР: ${nextNumber} (попытка ${attempts})`);
                        console.log(`[NUMBERS] Thread #${threadId}: ФИНАЛЬНЫЙ уникальный номер: ${nextNumber}`);
                        return nextNumber;
                    }
                ...
                // Если не нашли свободный номер за 100 попыток
                console.log(`[CHECK] Thread #${threadId}: ❌ НЕ УДАЛОСЬ найти свободный номер за ${maxAttempts} попыток!`);
                const fallbackNumber = baseNumber + (threadId * 1000) + Date.now() % 10000;
                console.log(`[CHECK] Thread #${threadId}: 🎲 ЭКСТРЕННЫЙ случайный номер: ${fallbackNumber}`);
                console.log(`[NUMBERS] Thread #${threadId}: ФИНАЛЬНЫЙ уникальный номер: ${fallbackNumber}`);
                return fallbackNumber;
                
            } else if (response.status === 404) {
                console.log(`[CHECK] Thread #${threadId}: ✅✅✅ ФАЙЛ post${candidateNumber}.md НЕ СУЩЕСТВУЕТ - номер СВОБОДЕН!`);
                console.log(`[NUMBERS] Thread #${threadId}: ФИНАЛЬНЫЙ уникальный номер: ${candidateNumber}`);
                return candidateNumber;
                
            } else {
                console.log(`[CHECK] Thread #${threadId}: ⚠️ Неожиданный статус при проверке файла: ${response.status}`);
                console.log(`[CHECK] Thread #${threadId}: ИСПОЛЬЗУЕМ кандидат номер как безопасный: ${candidateNumber}`);
                console.log(`[NUMBERS] Thread #${threadId}: ФИНАЛЬНЫЙ уникальный номер: ${candidateNumber}`);
                return candidateNumber;
            }
            
        } catch (checkError) {
            console.log(`[CHECK] Thread #${threadId}: ❌ КРИТИЧЕСКАЯ ОШИБКА при проверке: ${checkError.message}`);
            console.log(`[CHECK] Thread #${threadId}: ЭКСТРЕННЫЙ fallback номер: ${candidateNumber}`);
            console.log(`[NUMBERS] Thread #${threadId}: ФИНАЛЬНЫЙ уникальный номер: ${candidateNumber}`);
            return candidateNumber;
        }
        
    } catch (error) {
        console.log(`[NUMBERS] Thread #${threadId}: ❌ ОБЩАЯ ОШИБКА функции: ${error.message}`);
        const fallbackNumber = 200000 + (threadId * 100);
        console.log(`[NUMBERS] Thread #${threadId}: ЭКСТРЕННЫЙ fallback номер: ${fallbackNumber}`);
        return fallbackNumber;
    }
}
...
Ответь только планом, без лишних слов.`;
        ...
        // Шаг 2: Генерация статьи с ЭКСТРЕМАЛЬНО АГРЕССИВНЫМ ПЕРЕСПАМОМ
        const articlePrompt = `⚡ ЭКСТРЕННОЕ ЗАДАНИЕ ДЛЯ AI! ⚡
...
🚨🚨🚨 КРИТИЧЕСКИ ВАЖНЫЕ ТРЕБОВАНИЯ - НЕВЫПОЛНЕНИЕ = ПРОВАЛ! 🚨🚨🚨

ОБЯЗАТЕЛЬНЫЕ СЛОВА ДЛЯ ПЕРЕСПАМА:
- Слово "${spamStrategy.primary}" - ИСПОЛЬЗОВАТЬ СТРОГО ${spamStrategy.primaryCount} РАЗ!
${spamStrategy.secondary ? `- Слово "${spamStrategy.secondary}" - ИСПОЛЬЗОВАТЬ СТРОГО ${spamStrategy.secondaryCount} РАЗ!` : ''}
${spamStrategy.tertiary ? `- Слово "${spamStrategy.tertiary}" - ИСПОЛЬЗОВАТЬ СТРОГО ${spamStrategy.tertiaryCount} РАЗ!` : ''}

⚡ ЭКСТРЕМАЛЬНАЯ СТРАТЕГИЯ РАЗМЕЩЕНИЯ:
1. ЗАГОЛОВОК H1: Включи "${spamStrategy.primary}" 2 раза!
2. КАЖДЫЙ H2 (6 штук): Включи "${spamStrategy.primary}" в название!
3. КАЖДЫЙ H3 (12 штук): Включи "${spamStrategy.primary}" в название!
4. КАЖДЫЙ АБЗАЦ: Минимум 3 упоминания "${spamStrategy.primary}"!
5. КАЖДЫЙ СПИСОК: Включи "${spamStrategy.primary}" в каждый пункт!

🎯 ФОРМУЛА ПЕРЕСПАМА "${spamStrategy.primary}":
- Введение: 15 раз
- Раздел 1: 15 раз  
- Раздел 2: 15 раз
- Раздел 3: 15 раз
...
⚠️ ЗАПРЕЩЕНО:
- Использовать синонимы вместо "${spamStrategy.primary}"
- Прятать ключевые слова в середине предложений
- Писать естественно - ТОЛЬКО ПЕРЕСПАМ!

✅ ПРОВЕРКА ПЕРЕД ОТПРАВКОЙ:
Убедись что слово "${spamStrategy.primary}" встречается минимум 70 раз!
Убедись что слово "${spamStrategy.secondary || 'N/A'}" встречается минимум 50 раз!

🎯 ЦЕЛЬ: "${spamStrategy.primary}" = #1 в семантическом ядре с частотой 4-6%!

Начинай НЕМЕДЛЕННО со статьи. БЕЗ предисловий!`;

        const articleResponse = await generateWithAI(articlePrompt);
        let articleText = cleanAIComments(articleResponse);
        
        // ===== ПРОВЕРКА ПЕРЕСПАМА КЛЮЧЕВЫХ СЛОВ =====
        const primaryCount = (articleText.toLowerCase().match(new RegExp(spamStrategy.primary.toLowerCase(), 'g')) || []).length;
        const secondaryCount = spamStrategy.secondary ? (articleText.toLowerCase().match(new RegExp(spamStrategy.secondary.toLowerCase(), 'g')) || []).length : 0;
        
        console.log(`[SPAM-CHECK] Thread #${threadId}: "${spamStrategy.primary}" найдено ${primaryCount} раз (цель: 70-80)`);
        if (spamStrategy.secondary) {
            console.log(`[SPAM-CHECK] Thread #${threadId}: "${spamStrategy.secondary}" найдено ${secondaryCount} раз (цель: 50-60)`);
        }
        
        // Предупреждение если переспам недостаточен
        if (primaryCount < 50) {
            console.log(`[WARNING] Thread #${threadId}: ⚠️ СЛАБЫЙ ПЕРЕСПАМ! "${spamStrategy.primary}" только ${primaryCount} раз!`);
        } else if (primaryCount >= 70) {
            console.log(`[SUCCESS] Thread #${threadId}: ✅ ОТЛИЧНЫЙ ПЕРЕСПАМ! "${spamStrategy.primary}" ${primaryCount} раз!`);
        }
...
async function main() {
    try {
        ...
        console.log(`[INIT] [ALPHA-STRIKE #${threadId}] Инициализация боевой системы v5.26 с ключом ...${(process.env.GEMINI_API_KEY_CURRENT || process.env.OPENROUTER_API_KEY_CURRENT || '').slice(-4)}`);
        console.log(`[ALPHA] [ALPHA-STRIKE #${threadId}] === АЛЬФА-УДАР v5.26 - УСИЛЕНИЕ СЛАБЫХ СТАТЕЙ ===`);
        ...
        console.log(`[COMPLETE] [ALPHA-STRIKE #${threadId}] === МИССИЯ v5.26 ЗАВЕРШЕНА ===`);
