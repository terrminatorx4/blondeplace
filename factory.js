...
ОБЪЕМ: минимум 15000 символов - это критически важно!`;
    ...
    const seoPrompt = `Для статьи на тему "${topic}" сгенерируй JSON-объект. ВАЖНО: твой ответ должен быть ТОЛЬКО валидным JSON-объектом. 

JSON должен содержать: 
- "title" (длиной ровно 40-45 символов)
- "description" (длиной ровно 150-164 символа) 
- "keywords" (строка с 5-7 релевантными ключевыми словами через запятую, СТРОГО связанными с темой "${topic}")

ВАЖНО для keywords: используй только термины связанные с темой статьи. Например:
- Для статьи о криотерапии: "криотерапия, омоложение, холодовая терапия, anti age"
- Для статьи о маникюре: "маникюр, nail art, гель лак, уход за ногтями"

Контекст: это блог салона красоты ${BRAND_NAME}.`;
