...
Контекст: статья для блога салона красоты ${BRAND_NAME}, целевая аудитория - женщины 25-45 лет, интересующиеся красотой.`;
    ...
    const articlePrompt = `Напиши исчерпывающую, экспертную статью объемом минимум 12000 символов (для снижения тошноты)
...
- ВАЖНОТ: Щ повторять слова! Тошнота СТ до 4.5! аждое предложение - уникальные слова, синонимы, перифразы. бзацы максимально разнообразные!

ОБЪЕМ: минимум 12000 символов (для снижения тошноты)
...
- "title" (СТ 40-43 символа, включи основное ключевое слово)
- "description" (СТ 155-160 символов, продающий, с призывом к действию)
