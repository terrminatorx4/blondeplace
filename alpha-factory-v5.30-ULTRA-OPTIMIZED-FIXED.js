// ===== ALPHA-FACTORY v5.30-ULTRA-OPTIMIZED-FIXED - ИСПРАВЛЕНА ПЕРЕЗАПИСЬ =====
// КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Стартовый номер 30000+ для избежания конфликтов!
// НОВАЯ БЛОЧНАЯ СИСТЕМА: Thread 1: 30000-30999, Thread 2: 31000-31999, и т.д.
// РЕЗУЛЬТАТ: 100% API квота идет на генерацию контента, 0 перезаписей!
...
// ===== ИСПРАВЛЕННАЯ ФУНКЦИЯ НУМЕРАЦИИ БЕЗ ПЕРЕЗАПИСИ =====
function getSafePostNumber(threadId, articleIndex) {
    // БЕЗОПАСНАЯ БЛОЧНАЯ СИСТЕМА: начинаем с 30000 чтобы избежать всех существующих!
    // Thread 1: 30000-30999, Thread 2: 31000-31999, Thread 3: 32000-32999, и т.д.
    // НИКАКИХ проверок, НИКАКИХ API запросов, НИКАКИХ конфликтов!
    const safeStartNumber = 30000; // БЕЗОПАСНЫЙ старт выше ВСЕХ существующих статей
    const startNumber = safeStartNumber + (threadId * 1000) + articleIndex;
    
    console.log(`[SAFE-NUMBERS] Thread #${threadId}: 🛡️ БЕЗОПАСНАЯ НУМЕРАЦИЯ!`);
    console.log(`[SAFE-NUMBERS] Thread #${threadId}: Блок потока: ${safeStartNumber + (threadId * 1000)}-${safeStartNumber + (threadId * 1000) + 999}`);
    console.log(`[SAFE-NUMBERS] Thread #${threadId}: Статья ${articleIndex} → номер ${startNumber}`);
    console.log(`[SAFE-NUMBERS] Thread #${threadId}: ⚡ МГНОВЕННО! 0 API запросов! 0 задержек!`);
    console.log(`[SAFE-NUMBERS] Thread #${threadId}: 🚫 НЕТ ПЕРЕЗАПИСИ! Выше всех существующих!`);
...
async function main() {
    ...
    console.log(`🛡️ НУМЕРАЦИЯ: БЕЗОПАСНАЯ (30000+ без перезаписи!)`);
    ...
    console.log(`[INIT] [ALPHA-STRIKE #${threadId}] Инициализация боевой системы v5.30-FIXED с ключом ...${geminiKey.slice(-4)}`);
    
    console.log(`[ALPHA] [ALPHA-STRIKE #${threadId}] === АЛЬФА-УДАР v5.30-FIXED - ИСПРАВЛЕНА ПЕРЕЗАПИСЬ ===`);
    ...
    // БЕЗОПАСНАЯ СИСТЕМА НУМЕРАЦИИ БЕЗ ПЕРЕЗАПИСИ
    const startNumber = getSafePostNumber(threadId, 0);
    console.log(`[SAFE-NUMBERS] Thread #${threadId}: 🛡️ НАЧИНАЮ С БЕЗОПАСНОГО НОМЕРА: ${startNumber}`);
    ...
    for (let i = 0; i < targetArticles; i++) {
        ...
        const postNumber = getSafePostNumber(threadId, i);
    ...
    console.log(`[COMPLETE] [ALPHA-STRIKE #${threadId}] === МИССИЯ v5.30-FIXED ЗАВЕРШЕНА ===`);
