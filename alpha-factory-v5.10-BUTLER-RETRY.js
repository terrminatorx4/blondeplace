// ===== ALPHA-FACTORY v5.20 - БАЛАНС КЛЮЧЕВЫХ СЛОВ И ТОШНОТЫ =====
// НОВАЯ СТРАТЕГИЯ: КЛЮЧЕВЫЕ СЛОВА В ТОП-3, ТОШНОТА ПОД КОНТРОЛЕМ!
...
// 6. Объем: 17000+ символов (ВОЗВРАТ к рабочему объему) ✅
// 7. Ключевые слова: В ТОП-3 семантического ядра (3-8%) ✅
// 8. Тошнота: ЦЕЛЬ 4.5-5.0 через разнообразие контента ✅
...
Ответь только планом, без лишних слов.`;
        ...
        // Шаг 2: Генерация статьи с БАЛАНСОМ ключевых слов и тошноты
...
- ОБЯЗАТЕЛЬНО используй "${keyword}" 25-35 раз в статье (для попадания в ТОП-3 семантического ядра)
- Размещай "${keyword}" естественно в заголовках, первых абзацах разделов, заключении
- Варьируй формы: "${keyword}", "${keyword.replace(/\w+/, (match) => match.toLowerCase())}", связанные термины
- Цель: "${keyword}" должно быть на 1-3 месте в семантическом ядре с частотой 3-6%

📚 СТРУКТУРА ДЛЯ СНИЖЕНИЯ ТОШНОТЫ:
1. Введение (использовать "${keyword}" 3-4 раза)
2. Основная часть (распределить "${keyword}" равномерно по разделам)
3. Практические советы (включить "${keyword}" в каждый совет)
4. Детальные кейсы и примеры (2000+ символов общего текста)
5. Сравнительная таблица (500+ символов без ключевых слов)
6. Часто задаваемые вопросы (1000+ символов с редким использованием "${keyword}")
7. Заключение (использовать "${keyword}" 2-3 раза)

📖 ТРЕБОВАНИЯ К ОБЪЕМУ И РАЗНООБРАЗИЮ:
- ОБЯЗАТЕЛЬНО 17000+ символов 
- Используй синонимы: "услуга", "предложение", "вариант", "решение", "сервис"
- Добавляй технические детали, статистику, исторические факты
- Включай цитаты экспертов (выдуманные)
- Пиши разнообразно: короткие и длинные предложения
- Используй списки, таблицы, подзаголовки для структурирования

🔄 КОНТРОЛЬ ТОШНОТЫ:
- НЕ используй "${keyword}" в каждом предложении
- Разбавляй синонимами и общими терминами
- Добавляй "водный" контент: история, тренды, технологии
- Пиши от лица эксперта beauty-индустрии
- Включай профессиональную терминологию

ЦЕЛЬ: "${keyword}" в ТОП-3 семантического ядра + тошнота 4.5-5.0!
...
async function main() {
    try {
        ...
        console.log(`[INIT] [ALPHA-STRIKE #${threadId}] Инициализация боевой системы v5.20 с ключом ...${(process.env.GEMINI_API_KEY_CURRENT || process.env.OPENROUTER_API_KEY_CURRENT || '').slice(-4)}`);
        console.log(`[ALPHA] [ALPHA-STRIKE #${threadId}] === АЛЬФА-УДАР v5.20 - БАЛАНС КЛЮЧЕВЫХ СЛОВ ===`);
        ...
        console.log(`[SEO] [ALPHA-STRIKE #${threadId}] НОВАЯ СТРАТЕГИЯ: Ключевые слова в ТОП-3, объем 17000+`);
        ...
        console.log(`[COMPLETE] [ALPHA-STRIKE #${threadId}] === МИССИЯ v5.20 ЗАВЕРШЕНА ===`);
